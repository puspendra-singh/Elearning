...
function Transaction() {
	 ...
	 * @return render/json
	 */
	this.getTransactionList = (req,res)=>{
		...
		if(isPost(req)){
				...
				asyncParallel({
					records :(callback)=>{
						...
						collection.aggregate([
							...
							{$project: {
								_id:1,
								name : {$arrayElemAt:["$userDetail.full_name",0]},
								payment_status:1,amount:1,modified:1
							}},
					...
					total_records:(callback)=>{
						/** Get total number of records in  transacations collection **/
						collection.countDocuments(commonConditions,(err,countResult)=>{
							callback(err, countResult);
						});
					},
					filter_records:(callback)=>{
						/** Get filtered records counting in  transacations **/
						collection.aggregate([
							{$lookup:{
								from: "users",
								let: { userId: "$user_id"},
							...
							callback(err, filterContResult.length);
						});
					}
				},(err, response)=>{
					/** Send response **/
			...
			req.breadcrumbs(BREADCRUMBS['admin/transactions/list']);
			res.render('list',{
				type_name : type
			});
		}
